...
const primaryCards = [
  {
    title: "Deep Dive",
    eyebrow: "For the nerds",
    copy: "Track map, race story, head to head, and all the bits that explain why the order changed.",
    href: "/f1",
  },
  {
    title: "Quick Results",
    eyebrow: "Need it fast?",
    copy: "Latest result, driver standings, constructor table. In, out, informed.",
    href: "/f1/standings",
  },
] as const;
...
export default function LandingHero({ nextRace }: LandingHeroProps) {
  return (
    <header id="hero" className="relative flex min-h-[92vh] flex-col justify-center overflow-hidden bg-transparent pt-24">
      <div className="absolute inset-0 z-0 overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,rgba(180,20,0,0.34)_0%,#000_72%)]" />
        <Image
          alt="Formula 1 racing car"
          className="h-full w-full object-cover opacity-30 contrast-125 saturate-110"
          src="/images/f1-hero.jpg"
          fill
          priority
          sizes="100vw"
        />
        <div className="absolute inset-0 bg-[linear-gradient(180deg,rgba(0,0,0,0.48)_0%,rgba(0,0,0,0.72)_42%,rgba(0,0,0,0.92)_100%)]" />
        ...
        <div className="hero-scan absolute inset-x-0 top-0 h-40 bg-[linear-gradient(180deg,transparent_0%,rgba(225,6,0,0.18)_45%,transparent_100%)]" />
        <div className="absolute -left-20 top-24 h-72 w-72 rounded-full bg-primary/16 blur-[120px]" />
        <div className="absolute bottom-10 right-0 h-80 w-80 rounded-full bg-[#6F0D08]/18 blur-[140px]" />
        <div className="hero-track-drift absolute inset-x-[-8%] top-[12%] hidden h-[58%] lg:block">
      ...
      <div className="relative z-20 container mx-auto px-4 pb-12 md:pb-16">
        <div className="grid gap-12 lg:grid-cols-[minmax(0,1.1fr)_minmax(360px,0.9fr)] lg:items-end">
          <div className="max-w-3xl">
            <h1 className="hero-card-reveal mt-4 font-display text-6xl font-black italic leading-[0.9] tracking-tighter text-white drop-shadow-[0_0_28px_rgba(225,6,0,0.58)] md:text-8xl lg:text-[8.5rem]" style={{ animationDelay: "160ms" }}>
              THE GRID
            </h1>
            <p className="hero-card-reveal mt-6 max-w-2xl text-lg leading-relaxed text-gray-300 md:text-xl" style={{ animationDelay: "240ms" }}>
              Drivers, circuits, standings, and the whole weekend story without the fluff.
            </p>
            <div className="hero-card-reveal mt-8 inline-flex max-w-full flex-wrap items-center gap-3 rounded-2xl border border-white/10 bg-black/25 px-4 py-3 text-left" style={{ animationDelay: "320ms" }}>
                ...
                {nextRace ? (
                  <>
                    ...
                    <LocalDateTimeText
                      ...
                      fallback={`${nextRace.raceName} loading`}
                    />
                  </>
                ) : (
                  "Season data loading"
            ...
            <div className="relative grid gap-4">
                  ...
                  >
                    ...
                    <div className="relative flex h-full flex-col justify-between gap-8">
                      ...
                      <div className="flex items-center justify-between text-xs font-semibold uppercase tracking-[0.18em] text-gray-400">
                        <span>Open</span>
                        <span className="material-icons text-base text-white transition-transform duration-300 group-hover:translate-x-1">arrow_forward</span>
                      </div>
                    </div>
                  </Link>
                ))}
              ...
              >
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  <div>
                    <h3 className="mt-2 font-display text-2xl font-black italic text-white">
