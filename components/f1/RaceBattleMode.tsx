      ...
      <div className="border-t border-white/10 p-4 sm:p-5">
        <div className="grid gap-3 lg:grid-cols-2">
          <label className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-gray-500">Driver A</p>
            <select
              className="mt-1 min-h-11 w-full truncate bg-transparent pr-6 text-sm font-bold text-white outline-none"
          ...
          <label className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-gray-500">Driver B</p>
            <select
              className="mt-1 min-h-11 w-full truncate bg-transparent pr-6 text-sm font-bold text-white outline-none"
          ...
          <>
            <div className="mt-4 grid grid-cols-2 gap-3">
              <div className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-3 sm:p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">Finish gap</p>
              ...
              <div className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-3 sm:p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">Avg lap delta</p>
                ...
                <p className="mt-1 text-xs text-gray-500">{lapDeltas.length} shared laps</p>
              </div>
              <div className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-3 sm:p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">{leftTrace.code} faster laps</p>
                <p className="mt-2 text-xl font-black text-white">{leftFasterLaps}</p>
              </div>
              <div className="min-w-0 rounded-lg border border-white/10 bg-black/20 p-3 sm:p-4">
                <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">{rightTrace.code} faster laps</p>
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-3">
              {segmentLabels.map((segment) => (
                <div key={segment.label} className="min-w-0 rounded-lg border border-white/10 bg-black/20 px-4 py-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.16em] text-gray-500">{segment.label}</p>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-4">
              ...
              <div className="mt-4 max-h-[260px] space-y-2 overflow-y-auto pr-1 custom-scrollbar">
                {lapDeltas.map((lap) => {
                  ...
                  return (
                      <div key={lap.lap} className="grid grid-cols-[40px_minmax(0,1fr)_64px] items-center gap-2 sm:grid-cols-[46px_minmax(0,1fr)_82px] sm:gap-3">
                      <p className="text-xs font-semibold uppercase tracking-[0.14em] text-gray-500">Lap {lap.lap}</p>
                      ...
                      <p className="text-right text-xs font-semibold text-gray-300">{formatGap(lap.deltaMs)}</p>
                    </div>
                  );
                })}
              </div>
